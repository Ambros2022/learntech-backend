...
const getPagingData = (data, page, limit) => {
    const { count: totalItems, rows: scholartypes } = data;
    const currentPage = page ? +page : 1;
    const totalPages = Math.ceil(totalItems / limit);
    return { totalItems, scholartypes, totalPages, currentPage };
};

exports.create = async (req, res) => {

    try {
        const scholartypesDetails = await scholartypes.create({
            name: req.body.name,
        ...
        .catch(err => {
            res.status(500).send({
                ...
                message:
                    ...
                    err.message || "Some error occurred while retrieving scholar types."
            });
        });
};
...
exports.findOne = (req, res) => {
    ...
    scholartypes
        ...
        .catch((err) => {
            res.status(500).send({
                status: 0,
                message: "Error retrieving scholar types with id=" + id,
            });
        });
};
